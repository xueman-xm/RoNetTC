...
class MobileViT(nn.Module):
    ...
    def __init__(self, model_cfg: Dict, num_classes: int = 1000):
        ...
        self.last_classifier.add_module(name="global_pool", module=nn.AdaptiveAvgPool2d(1))
        self.last_classifier.add_module(name="flatten", module=nn.Flatten())
        if 0.0 < model_cfg["cls_dropout"] < 1.0:
            self.last_classifier.add_module(name="dropout", module=nn.Dropout(p=model_cfg["cls_dropout"]))
        self.last_classifier.add_module(name="fc", module=nn.Linear(in_features=exp_channels, out_features=num_classes))
        self.last_classifier.add_module(name="act", module=nn.Softplus())
...
def mobile_vit_xx_small(num_classes: int = 12):
